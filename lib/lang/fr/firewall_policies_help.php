...
<p><img src="static/bullet_off.gif">&nbsp;<strong>Autoriser les téléchargements :</strong> vous pouvez autoriser ou interdire les téléchargements de fichier vers votre site, ou bien les autoriser sauf s'il s'agit de fichiers potentiellement dangereux&nbsp;: scripts (PHP, CGI, Ruby, Python, bash/shell), code source C/C++, fichiers exécutables (MZ/PE/NE et ELF), certains fichiers systèmes (<code>.htaccess</code>, <code>.htpasswd</code> et PHP INI) et les fichiers SVG contenant du Javascript ou des Événements XML.</p>
<p><img src="static/bullet_off.gif">&nbsp;<strong>Nettoyer le nom des fichiers :</strong> si le nom du fichier contient un caractère qui n'est pas une lettre <code>a-zA-Z</code>, un chiffre <code>0-9</code>, un point <code>.</code>, un trait d'union <code>-</code> ou un caractère de soulignement <code>_</code>, celui-si sera remplacé par le caractère de substitution.</p>
...
<p><img src="static/bullet_off.gif">&nbsp;<strong>Bloquer les objets PHP sérialisés&nbsp;:</strong> La sérialisation est utilisée par de nombreuses applications afin de générer une représentation stockable d'une valeur. Cependant, lorsqu'elle est utilisée dans des applications, thèmes ou extensions mal sécurisées, la sérialisation d'objets peut être exploitée pour injecter et exécuter du code PHP (PHP Object Injection). NinjaFirewall peut bloquer les objets PHP sérialisés se trouvant dans les requêtes <code>GET</code> ou <code>POST</code>, les cookies, ou dans les variables <code>HTTP_REFERER</code> et <code>HTTP_USER_AGENT</code>.</p>
...
<p><img src="static/bullet_off.gif">&nbsp;<strong>Bloquer les caractères de contrôle ASCII 1 à 8 et 14 à 31&nbsp;:</strong> toute requête <code>GET</code> ou <code>POST</code>, ainsi que toute variable <code>HTTP_USER_AGENT</code>, <code>HTTP_REFERER</code> contenant des caractères ASCII 1 à 8 et 14 à 31 sera bloquée.
